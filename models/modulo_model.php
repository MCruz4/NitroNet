...
class ModuloModel extends DB{
    ...
    function getAllData(){   
        ...
        $stmt = $conn->prepare('SELECT * FROM modulo');
    ...
    function getByIdData(int $id){   
        ...
        // Preparamos la consulta
            $stmt = $conn->prepare('SELECT * FROM modulo WHERE id_modulo=:id_modulo');     
        // Trataremos de ejecutar
        try{
            // Cargamos los parametros que utilizaremos.
            $stmt->bindParam(':id_modulo', $id, PDO::PARAM_INT, 11);
            // Ejecutamos la consulta
            $stmt->execute();
            // Retornamos los valores obtenidos
            return $stmt;
    ...
    function insertOneData($array){
        ...
        $conn = $this->connect();
        //Preparamos la consulta
        $stmt = $conn->prepare("INSERT INTO modulo VALUES (
            NULL , 
            :nom_modulo ,
            :desc_modulo,
            :visible_modulo);");
        try{
            ...
            $stmt->bindParam(':nom_modulo', $array['nom_modulo'], PDO::PARAM_STR, 50);
            $stmt->bindParam(':desc_modulo', $array['desc_modulo'], PDO::PARAM_STR, 150);
            $stmt->bindParam(':visible_modulo', $array['visible_modulo'], PDO::PARAM_STR , int);
            // Ejecutamos la consulta   
            $stmt->execute();
